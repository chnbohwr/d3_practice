/*global -$ */
'use strict';
// generated on 2018-03-13 using generator-modern-frontend 0.2.9
var fs = require('fs');
...
gulp.task('stylesheet', function () {
  return gulp.src('app/css/main.less')
    ...
    .on('error', function (error) {
      console.log('compile less got error', error);
      this.emit('end');
    })
    .pipe($.postcss([
...
gulp.task('serve', ['stylesheet', 'javascript', 'fonts', 'csv'], function () {
  browserSync({
    notify: false,
    port: 9000,
    server: {
  ...
  gulp.watch(['app/css/**/*.less'], ['stylesheet']);
  gulp.watch('app/js/**/*.{js,jsx}', ['javascript']);
  gulp.watch('app/fonts/**/*', ['fonts']);
  gulp.watch('app/csv/*', ['csv'])
});
